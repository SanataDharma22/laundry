...
$pengguna = query("SELECT * FROM tb_user WHERE role = 'owner'");
?>
                ...
                <?php foreach($pengguna as $s) : ?>
                  <tr>
                    ...
                    <td><a class='btn btn-primary' href="update_owner.php?id=<?php echo $s["id"]; ?>"><i class='icon-edit'></i>Edit</a>
                      <a href="hapus_owner.php?id=<?php echo $s["id"]; ?>" class='btn btn-danger' onclick="return confirm('Anda yakin ingin menghapus ini?');"><i class='icon-trash'></i>Hapus</td>
